...
process<-function(cameras, path, codepath, t1, t2, t3){
 ...
 print(cameraname)
  ...
  foldersetup<-paste(codepath, "1 sw setup folders.R", sep="")
  ...
  ## 3) Extract the metadata to a data file in the metadata folder
  metadataextractpath<-paste(codepath, "3 sw metadata extraction.R", sep="") #This is for reconyx and bushnell images 
  ...
  ## 4) Reduce image size and select three pictures per day 
  # run A), B), C), OR D), do not run all of them (put a # in front of the line you are not running)
  # 4A) Select and reduce three reconyx or time-lapse systems images
  #select3adaypath<-paste(codepath, "/4a sw select and reduce zooniverse.R", sep="")
  # 4B) Select and reduce three bushnell images
  #select3adaypath<-paste(codepath, "/4b sw select and reduce zooniverse_round minutes bushnell.R", sep="") # Round minutes down to nearest hour, for when Bushnell cameras took images at 'random' times (e.g. 11:13:12, 10:49:00, 18:17:11 will become 11:00:00, 10:00:00, 18:00:00)
  # 4C) Select three reconyx or time-lapse systems images but do not reduce, keep at original size
  select3adaypath<-paste(codepath, "/4c sw select zooniverse.R", sep="") # Keep original image size and select three pictures a day
  # 4D) Select three bushneel images but do not reduce, keep at original size 
  #select3adaypath<-paste(codepath, "/4d sw select zooniverse_round minutes bushnell.R", sep="") # Keep original image size and select three pictures a day; Round minutes down to nearest hour, for when Bushnell cameras took images at 'random' times (e.g. 11:13:12, 10:49:00, 18:17:11 will become 11:00:00, 10:00:00, 18:00:00)
  ...
  # 5B) Select the midday images for bushnell cameras
  #selectmiddaypath<-paste(codepath, "5b sw select midday_round minutes bushnell.R", sep="") # Round minutes down to nearest hour, for when Bushnell cameras took images at 'random' times (e.g. 11:13:12, 10:49:00, 18:17:11 will become 11:00:00, 10:00:00, 18:00:00)
