...
outdir <- "C:/Users/Ladmin/Documents/1 Seabird Watch/Data/2 Processed classifications/Kittiwake nests/"
...
sw_class <- sw_classifications[grep("Kittiwake nests", sw_classifications$workflow_name), ] 
...
adult <- sw_class[grep(pattern="Adult kittiwake", x=sw_class$annotations),]

#-- Chick tool 
# select any row with 'Adult' in annotations column
chick <- sw_class[grep(pattern="Chick kittiwake", x=sw_class$annotations),]
...
write.csv(adult_xy, paste0(outdir, "Kittiwake nests adult_citsciRaw.csv"), row.names=FALSE)
write.csv(chick_xy, paste0(outdir, "Kittiwake nests chick_citsciRaw.csv"), row.names=FALSE)

# If want to start the code from this point and read in the above files
#adult <-read.csv(paste0(outdir, "Kittiwake nests adult_citsciRaw.csv"), row.names=1) #row.names=1 means the row names in the csv (i.e. 1, 2, 3...) become row names in the table, rather than being the first column of the dataset
#chick <-read.csv(paste0(outdir, "Kittiwake nests chick_citsciRaw.csv"), row.names=1) 
...
FOWLa <- merge_click_meta(cluster_data = list(adult_xy, chick_xy), 
                          tools = c("adult_kitt", "chick_kitt"),
                          meta_inputdir = meta_indir,
                          site = "FOWLa",
                          siteyear = c("FOWLa2022a", "FOWLa2022b", "FOWLa2022c", "FOWLa2022d"), 
                          site_save="FOWLa",
                          outputdir = outdir,
                          datatype="citsciRaw")

MARWa <- merge_click_meta(cluster_data = list(adult_xy, chick_xy), 
                          tools = c("adult_kitt", "chick_kitt"),
                          meta_inputdir = meta_indir,
                          site = "MARWa",
                          siteyear = c("MARWa2022a", "MARWa2022b"), 
                          site_save="MARWa",
                          outputdir = outdir,
                          datatype="citsciRaw")
